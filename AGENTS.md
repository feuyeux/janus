...
This is a Java 25 Maven service for the Janus server. Source code lives under `src/main/java/org/janus/`, organized by concern: `codec/` for binary encoding, `common/` for shared helpers, `config/`, `discovery/` for Nacos/etcd/static integration and gRPC name resolvers, `grpc/`, `handler/`, `model/`, `observability/`, `security/` for TLS, and `ws/`. Protocol definitions are in `src/main/proto/janus.proto`; generated protobuf/gRPC sources are produced under `target/generated-sources/` and should not be edited. Runtime configuration such as Log4j is in `src/main/resources/`. Architecture, protocol, usage, service-discovery, and load-balancing notes are in `doc/`. Container and observability assets are in `docker/` — `docker/docker-compose.yml` is the full S1→S2→S3 stack, and `docker/docker-compose.exp.yml` (plus `docker/nginx/`) is the standalone nginx load-balancing experiment.
...
## Testing Guidelines
Unit/integration tests live under `src/test/java`, mirroring the production package path — e.g. `codec/BinaryCodecTest`, `model/JanusMessageTest`, `common/HelloUtilsTest`, `handler/ChainHandlerLocalTest`, `grpc/HeaderInterceptorPropagationTest`, and `ws/WsForwardingConcurrencyTest`. New functionality should add tests here with names that describe behavior. Run `mvn test` before opening a pull request; use the Docker stack for end-to-end WebSocket, gRPC, discovery, and observability validation, and `docker/docker-compose.exp.yml` + `docker/nginx/validate.sh` for the nginx load-balancing / limiting / starvation / multi-instance experiments (see `doc/load-balance.md`).
